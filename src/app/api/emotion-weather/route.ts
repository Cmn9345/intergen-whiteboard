...
export async function GET(request: Request) {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/pb/emotion_weather/records?${params}`);
...
export async function POST(request: Request) {
  try {
    ...
    const checkRes = await fetch(`${API_BASE}/api/pb/emotion_weather/records?${checkParams}`);
    ...
    const res = await fetch(`${API_BASE}/api/pb/emotion_weather/records`, {
      method: 'POST',
